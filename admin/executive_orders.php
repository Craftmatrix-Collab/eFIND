...
// Function to validate if the file is an executive_order document (image only)
function isValidExecutiveOrderDocument($file) {
...
function generateReferenceNumber($conn, $executive_order_date = null) {
    if ($executive_order_date) {
        $year = date('Y', strtotime($executive_order_date));
        $month = date('m', strtotime($executive_order_date));
    } else {
        $year = date('Y');
        $month = date('m');
    }
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM executive_orders WHERE YEAR(executive_order_date) = ? AND MONTH(executive_order_date) = ?");
...
if (isset($_GET['print']) && $_GET['print'] === '1') {
    ...
    $printQuery .= " ORDER BY executive_order_date DESC";
    ...
    $printExecutiveOrders = $printResult->fetch_all(MYSQLI_ASSOC);
    $printStmt->close();
    ...
    <head>
        ...
        <title>Executive Orders Report - eFIND System</title>
    ...
    if (empty($printExecutiveOrders)) {
        ...
        foreach ($printExecutiveOrders as $executive_order) {
            ...
            echo '<tr>
                <td>' . $count . '</td>
                <!-- <td><span class="reference-number">' . htmlspecialchars($executive_order['reference_number'] ?? 'N/A') . '</span></td> -->
                <td>' . htmlspecialchars($executive_order['title']) . '</td>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_executive_order'])) {
        try {
            error_log("=== ADD EXECUTIVE ORDER START ===");
            // Add new executive_order
            ...
            if (isset($_FILES['image_file']) && !empty($_FILES['image_file']['name'][0])) {
                ...
                $image_paths = [];
                backfillDocumentImageHashes($conn, 'executive_order', 200);
            ...
            foreach ($_FILES['image_file']['tmp_name'] as $key => $tmpName) {
                ...
                if (!isValidExecutiveOrderDocument(['type' => $_FILES['image_file']['type'][$key], 'tmp_name' => $_FILES['image_file']['tmp_name'][$key]])) {
                ...
                $objectName = 'executive_orders/' . date('Y/m/') . $uniqueFileName;
        ...
        if ($stmt->execute()) {
            ...
            if (!empty($uploadedImageHashEntries)) {
                saveDocumentImageHashes($conn, 'executive_order', $new_executive_order_id, $uploadedImageHashEntries);
            }
            ...
            $_SESSION['error'] = "Failed to add executive_order: " . $conn->error;
        }
        $stmt->close();
        error_log("=== ADD EXECUTIVE ORDER END ===");
        header("Location: executive_orders.php");
        exit();
        } catch (Exception $e) {
            error_log("EXCEPTION in add_executive_order: " . $e->getMessage());
    ...
    if (isset($_POST['update_executive_order'])) {
        // Update existing executive_order
        $id = intval($_POST['executive_order_id']);
        $title = trim($_POST['title']);
        $executive_order_number = trim($_POST['executive_order_number']);
        $date_posted = $_POST['date_posted'];
        $executive_order_date = $_POST['executive_order_date'];
        ...
        if (isset($_FILES['image_file']) && !empty($_FILES['image_file']['name'][0])) {
            ...
            foreach ($_FILES['image_file']['tmp_name'] as $key => $tmpName) {
                ...
                $imageHash = computeAverageImageHashFromFile($tmpName);
                if ($imageHash !== null && $imageHash !== '') {
                    $duplicateMatches = findMatchingImageHashes($conn, 'executive_order', $imageHash, $id);
        ...
        if ($stmt->execute()) {
            ...
            if (function_exists('logDocumentUpdate')) {
                try {
                    logDocumentUpdate('executive_order', $title, $id, "Executive Order updated: $title");
                } catch (Exception $e) {
                    error_log("Logger error: " . $e->getMessage());
                }
            }
            $_SESSION['success'] = "Executive Order updated successfully!";
        } else {
            $_SESSION['error'] = "Failed to update executive_order: " . $conn->error;
        }
        $stmt->close();
        header("Location: executive_orders.php");
        exit();
...
// Handle GET request for fetching executive_order data
if (isset($_GET['action']) && $_GET['action'] === 'get_executive_order' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT * FROM executive_orders WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    $stmt->close();
    if ($executive_order) {
        logDocumentView('executive_order', $executive_order['title'] ?? 'Executive Order', $id);
    }
    header('Content-Type: application/json');
    echo json_encode($executive_order);
    exit();
}
...
$years_query = $conn->query("
    SELECT DISTINCT YEAR(executive_order_date) as year FROM executive_orders
    ORDER BY year DESC
");
...
$query = "SELECT id, title, description, executive_order_number, date_posted, executive_order_date, content, image_path, date_issued, file_path, uploaded_by,
    ...
    CASE WHEN EXISTS (
        ...
        WHERE dih.document_type = 'executive_order'
          AND dih.document_id = executive_orders.id
          AND dih.image_hash IN (
              SELECT image_hash
              FROM document_image_hashes
              WHERE document_type = 'executive_order'
              GROUP BY image_hash
...
$valid_sorts = [
    'executive_order_date_desc' => 'executive_order_date DESC',
    'executive_order_date_asc'  => 'executive_order_date ASC',
    'title_asc'            => 'title ASC',
...
$stmt->execute();
$result = $stmt->get_result();
$executive_orders = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
$stmt->close();

// Fetch total count for pagination
$count_query = "SELECT COUNT(*) as total FROM executive_orders";
if (!empty($where_clauses)) {
...
$total_executive_orders = $total_row ? $total_row['total'] : 0;
$total_pages = ceil($total_executive_orders / $table_limit);
$count_stmt->close();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Executive Orders Management - eFIND System</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <div class="management-container">
        <div class="container-fluid">
            ...
            <div class="page-header">
                <h1 class="page-title">Executive Orders Management</h1>
                            ...
                            <?php else: ?>
                                ...
                                <?php foreach ($executive_orders as $executive_order): ?>
                                    <tr data-id="<?php echo $executive_order['id']; ?>"<?php echo !empty($executive_order['has_duplicate_image']) ? ' style="background-color: #ffd8a8;"' : ''; ?>>
                                        ...
                                        <td>
                                            <?php if (!empty($executive_order['image_path'])): ?>
                                                <div class="d-flex gap-1 justify-content-center">
                                                    <a href="#" class="btn btn-sm btn-outline-success p-1 image-link" data-image-src="<?php echo htmlspecialchars($executive_order['image_path']); ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="View Image">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <button type="button" class="btn btn-sm btn-outline-warning p-1 ocr-btn" data-image-src="<?php echo htmlspecialchars($executive_order['image_path']); ?>" data-executive_order-id="<?php echo $executive_order['id']; ?>" data-bs-toggle="modal" data-bs-target="#ocrModal" data-bs-toggle="tooltip" data-bs-placement="top" title="OCR">
                                        ...
                                        <td>
                                            <div class="d-flex gap-1 justify-content-center">
                                                <button class="btn btn-sm btn-outline-primary p-1 edit-btn" data-id="<?php echo $executive_order['id']; ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-outline-danger p-1 delete-btn" data-id="<?php echo $executive_order['id']; ?>" data-title="<?php echo htmlspecialchars($executive_order['title']); ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete">
            ...
            <!-- Sticky Pagination -->
            <?php if ($total_executive_orders > 5): ?>
    ...
    <?php endif; ?>
    <!-- Modal for Add New Executive Order -->
    <div class="modal fade" id="addExecutiveOrderModal" tabindex="-1" aria-labelledby="addExecutiveOrderModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i> Add New Executive Order</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    ...
                    <form method="POST" action="" enctype="multipart/form-data" id="addExecutiveOrderForm">
                        <input type="hidden" name="allow_duplicate_images" id="allowDuplicateExecutiveOrderImages" value="0">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="executive_order_number" class="form-label">Executive Order Number <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="executive_order_number" name="executive_order_number" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="ord-mobile-upload" class="d-none">
                                <div class="text-center p-4 border rounded-3 bg-light">
                                    ...
                                    <p class="text-muted small mb-3">Point your phone's camera at this QR code to open the mobile upload page for executive_orders.</p>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" name="add_executive_order" class="btn btn-primary-custom">Add Executive Order</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <!-- Modal for Edit Executive Order -->
    <div class="modal fade" id="editExecutiveOrderModal" tabindex="-1" aria-labelledby="editExecutiveOrderModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-edit me-2"></i> Edit Executive Order</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editExecutiveOrderForm" method="POST" action="" enctype="multipart/form-data">
                        <input type="hidden" name="executive_order_id" id="editExecutiveOrderId">
                        <input type="hidden" name="existing_image_path" id="editExistingImagePath">
                        <input type="hidden" name="allow_duplicate_images" id="allowDuplicateExecutiveOrderEditImages" value="0">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="editExecutiveOrderNumber" class="form-label">Executive Order Number <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="editExecutiveOrderNumber" name="executive_order_number" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" name="update_executive_order" class="btn btn-primary-custom">Update Executive Order</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="modal fade ocr-modal" id="ocrModal" tabindex="-1" aria-labelledby="ocrModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input type="hidden" id="ocrExecutiveOrderId">
    ...
    <script>
    /* ── Executive Order Mobile Upload QR ── */
    (function () {
        ...
        async function ordEnsureSession() {
            ...
            const res = await fetch('mobile_session.php', {
                ...
                body: JSON.stringify({ doc_type: 'executive_orders' }),
            });
        ...
        async function ordApplyMobileImagesToForm(data) {
            ...
            const modalEl = document.getElementById('addExecutiveOrderModal');
            ...
            if (!fileInput) {
                throw new Error('Executive Order image input field not found.');
            }
            ...
            for (let i = 0; i < imageUrls.length; i++) {
                ...
                try {
                    ...
                    const file = new File([blob], `mobile_executive_order_${i + 1}.${ext}`, {
        ...
        async function ordHandleComplete(data) {
            ...
            try {
                ...
                console.error('Unable to apply mobile upload to executive_order form:', error);
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const modal = document.getElementById('addExecutiveOrderModal');
            if (modal) {
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const tableBody = document.querySelector('#executive_ordersTableBody');
            if (tableBody) {
...
<\/script>
            ...
            document.querySelectorAll('.edit-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    fetch(`?action=get_executive_order&id=${id}`)
                        .then(response => response.json())
                        .then(executive_order => {
                            document.getElementById('editExecutiveOrderId').value = executive_order.id;
                            document.getElementById('editTitle').value = executive_order.title;
                            ...
                            if (executive_order.image_path) {
                                const imagePaths = parseImageSources(executive_order.image_path);
                            ...
                            const editModal = new bootstrap.Modal(document.getElementById('editExecutiveOrderModal'));
                            editModal.show();
                        })
                        .catch(error => {
                            console.error('Error fetching executive_order:', error);
                            alert('Error loading executive_order data.');
                        });
                });
            });
            ...
            document.querySelectorAll('.ocr-btn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    var imageSrc = parseImageSources(btn.getAttribute('data-image-src'))[0] || '';
                    var executive_orderId = btn.closest('tr').getAttribute('data-id');
                    ...
                    document.getElementById('ocrExecutiveOrderId').value = executive_orderId;
                    ...
                    // First try to get existing content from the executive_order table
                    fetch(`?action=get_executive_order&id=${executive_orderId}`)
                        .then(response => response.json())
                        .then(executive_order => {
                            if (executive_order && executive_order.content) {
                                // Use existing content from the executive_order table
                                ocrLoading.style.display = 'none';
                                ocrResult.style.display = 'block';
                                ocrActions.style.display = 'flex';
                                document.getElementById('ocrTextEditor').value = executive_order.content;
                                formatOcrText(executive_order.content);
                                document.querySelector('#rawView pre').textContent = executive_order.content;
                            } else {
                                // If no content in executive_order table, check OCR table
                                fetch(`get_ocr_content.php?id=${executive_orderId}&type=executive_order`)
                                    .then(response => response.json())
                                    .then(data => {
                                        if (data.success && data.content) {
                                            ...
                                            performOCR(imageSrc, executive_orderId);
                                        }
                                    })
                                    .catch(() => {
                        ...
                        .catch(() => {
                            // If fetching executive_order fails, try OCR table
                            fetch(`get_ocr_content.php?id=${executive_orderId}&type=executive_order`)
                                .then(response => response.json())
                                .then(data => {
                                    if (data.success && data.content) {
            ...
            document.getElementById('saveOcrText').addEventListener('click', function() {
                ...
                const executive_orderId = document.getElementById('ocrExecutiveOrderId').value;
                const saveButton = this;
                const originalText = saveButton.innerHTML;
                // Validate input
                if (!executive_orderId || executive_orderId <= 0) {
                    alert('Error: Invalid executive_order ID');
                    return;
                }
                if (!editedText.trim()) {
                ...
                formData.append('id', executive_orderId);
                formData.append('document_type', 'executive_order');
        ...
        async function performOCR(imagePath, executive_orderId) {
            ...
            try {
                ...
                if (text && text.trim().length > 0) {
                    ...
                    saveOcrToDatabase(executive_orderId, cleanedText, typeof confidence === 'number' ? confidence : 0);
                } else {
                    ocrResult.style.display = 'block';
                    document.getElementById('formattedView').innerHTML = `
        ...
        function saveOcrToDatabase(executive_orderId, text, confidence) {
            if (!executive_orderId || !text) return;
            const formData = new FormData();
            formData.append('id', executive_orderId);
            formData.append('content', text);
            formData.append('confidence', confidence);
            formData.append('type', 'executive_order');
            fetch('update_document_content.php', {
                method: 'POST',
                body: formData
            })
        ...
        function analyzeDocumentContent(text) {
            ...
            // Extract executive_order number using various patterns
            const executive_orderPatterns = [
                /Executive Order\s+No\.?\s*([A-Z0-9\-]+)/i,
                /Executive Order\s+([A-Z0-9\-]+)/i,
                /Executive Order\s+Number\s+([A-Z0-9\-]+)/i,
                /EXECUTIVE\s+ORDER\s+NO\.?\s*([A-Z0-9\-]+)/i,
                /Resolution\s+No\.?\s*([A-Z0-9\-]+)/i,
                /RESOLUTION\s+NO\.?\s*([A-Z0-9\-]+)/i
            ];
            for (const pattern of executive_orderPatterns) {
                const match = text.match(pattern);
                if (match && match[1]) {
                    detectedFields.executive_orderNumber = match[1];
                    break;
                }
            }
        ...
        function updateFormWithDetectedData(detectedFields) {
            ...
            // Update executive_order number
            if (detectedFields.executive_orderNumber && !document.getElementById('executive_order_number').value) {
                document.getElementById('executive_order_number').value = detectedFields.executive_orderNumber;
                document.getElementById('executive_order_number').classList.add('field-highlight', 'detected-field');
            }
            // Update dates
            if (detectedFields.dates.length > 0) {
                const today = new Date().toISOString().split('T')[0];
                // Use first date for executive_order date if not set
                if (!document.getElementById('executive_order_date').value) {
                    const executive_orderDate = parseDate(detectedFields.dates[0]);
                    if (executive_orderDate) {
                        document.getElementById('executive_order_date').value = executive_orderDate;
                        document.getElementById('executive_order_date').classList.add('field-highlight', 'detected-field');
                    }
                }
        ...
        function showAutoFillResults(detectedFields, confidence) {
            ...
            if (detectedFields.executive_orderNumber) {
                resultsHTML += `
                    <div class="col-md-6 mb-2">
                        <div class="d-flex align-items-center">
                            <i class="fas fa-check text-success me-2"></i>
                            <small>Executive Order number detected</small>
                        </div>
                    </div>
                `;
            }
        ...
        function autoFillFormFields(text, formType = 'add') {
            ...
            const executive_orderNumberField = document.getElementById(prefix + 'ExecutiveOrderNumber');
            const datePostedField = document.getElementById(prefix + 'DatePosted');
            const executive_orderDateField = document.getElementById(prefix + 'ExecutiveOrderDate');
            ...
            if (foundDates.length >= 2) {
                ...
                if (executive_orderDateField) executive_orderDateField.value = foundDates[1];
            } else if (foundDates.length === 1) {
                if (datePostedField) datePostedField.value = foundDates[0];
                if (executive_orderDateField) executive_orderDateField.value = foundDates[0];
            }
            const executive_orderPatterns = [
                /Executive Order\s+No\.?\s*([A-Z0-9\-]+)/i,
                /Executive Order\s+([A-Z0-9\-]+)/i,
            ...
            for (const pattern of executive_orderPatterns) {
                const match = text.match(pattern);
                if (match && match[1] && executive_orderNumberField) {
                    executive_orderNumberField.value = match[1];
                    break;
                }
            }
        }
        ...
        document.getElementById('addExecutiveOrderModal').addEventListener('hide.bs.modal', function () {
