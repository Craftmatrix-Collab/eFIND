...
function ensureMinutesFullTextIndex($conn) {
    $indexName = 'idx_minutes_fulltext';
    $checkStmt = $conn->prepare("
        SELECT 1
        FROM information_schema.statistics
        WHERE table_schema = DATABASE()
          AND table_name = 'minutes_of_meeting'
          AND index_name = ?
        LIMIT 1
    ");

    if (!$checkStmt) {
        error_log("Failed to prepare minutes fulltext index check: " . $conn->error);
        return false;
    }
    ...
    $createIndexSql = "ALTER TABLE minutes_of_meeting
        ADD FULLTEXT INDEX idx_minutes_fulltext
        (title, reference_number, session_number, content, uploaded_by)";

    if (!$conn->query($createIndexSql)) {
        error_log("Failed to create minutes_of_meeting fulltext index: " . $conn->error);
        return false;
    }
...
if (isset($_GET['print']) && $_GET['print'] === '1') {
    ...
    $printParams = [];
    $printTypes = '';
    appendPrintDateRangeConditions('meeting_date', $printStartDate, $printEndDate, $printConditions, $printParams, $printTypes);
...
$fullTextMatchSql = "MATCH(title, reference_number, session_number, content, uploaded_by) AGAINST (? IN NATURAL LANGUAGE MODE)";

// Add search condition if search query is provided
if (!empty($search_query)) {
    $hasSearchToken = preg_match('/[\p{L}\p{N}]/u', $search_query) === 1;
    if ($search_length >= 2 && $hasSearchToken) {
        $searchVariants = buildTypoTolerantSearchVariants($conn, 'minutes', $search_query);
        if (empty($searchVariants)) {
            $searchVariants = [$search_query];
        }

        if ($hasMinutesFullTextIndex === null) {
            $hasMinutesFullTextIndex = ensureMinutesFullTextIndex($conn);
        }
        if ($hasMinutesFullTextIndex) {
                ...
                <div class="row g-2">
                    <div class="col-lg-7 col-md-6 col-12">
                        <div class="search-box">
                            ...
                            <input type="text" name="search_query" id="searchInput" class="form-control" placeholder="Search any minutes field (title, session no., content, etc.)..." value="<?php echo htmlspecialchars($search_query); ?>">
                        </div>
                    </div>
                    ...
                    <form method="POST" action="" enctype="multipart/form-data" id="addMinuteForm">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="mom-desktop-upload">
                                <div class="file-upload" id="momAddFileUploadZone">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editMinuteForm" method="POST" action="" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Image Files (JPG, PNG, GIF, BMP)</label>
                            <div class="file-upload" id="momEditFileUploadZone">
                                <div class="dropzone-instruction"><i class="fas fa-upload" aria-hidden="true"></i>Drop files here or click to browse</div>
...
<\/script>
                    ...
                    fetch(`?action=get_minute&id=${id}`)
                        ...
                        .then(minute => {
                            ...
                            if (minute.image_path) {
                                const imagePaths = parseImageSources(minute.image_path);
                                let imageLinks = '';
            ...
            bindDragDropUploadZone('momAddFileUploadZone', 'image_file', async function(input, options) {
                await processFilesWithAutoFill(input, options);
            });
            bindDragDropUploadZone('momEditFileUploadZone', 'editImageFile', async function(input, options) {
