...
include('includes/resend.php'); // Include Resend for 2FA
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        $query = "SELECT id, username, password_hash, full_name, profile_picture, email, two_fa_enabled FROM admin_users WHERE username = ?";
        ...
        if ($stmt) {
            ...
            if ($result->num_rows == 1) {
                ...
                if (password_verify($password, $user['password_hash'])) {
                    // Check if 2FA is enabled and email exists
                    $twoFaEnabled = isset($user['two_fa_enabled']) && $user['two_fa_enabled'] == 1;
                    $hasEmail = !empty($user['email']);
                    
                    if ($twoFaEnabled && $hasEmail) {
                        // Generate and send OTP
                        $otp = generateOTP();
                        
                        if (storeOTP($user['id'], $otp, 'admin') && sendOTPEmail($user['email'], $user['full_name'], $otp)) {
                            // Store pending login data in session
                            session_regenerate_id(true);
                            $_SESSION['pending_2fa_user_id'] = $user['id'];
                            $_SESSION['pending_2fa_user_type'] = 'admin';
                            $_SESSION['pending_admin_id'] = $user['id'];
                            $_SESSION['pending_admin_username'] = $user['username'];
                            $_SESSION['pending_admin_full_name'] = $user['full_name'];
                            $_SESSION['pending_admin_profile_picture'] = $user['profile_picture'];
                            $_SESSION['pending_user_email'] = $user['email'];
                            $_SESSION['pending_user_name'] = $user['full_name'];
                            
                            // Log 2FA initiated
                            logActivity($user['id'], '2fa_initiated', 'Two-factor authentication code sent', 'system', $user_ip);
                            ...
                            logActivity($user['id'], '2fa_send_failed', 'Failed to send 2FA code', 'system', $user_ip);
                        }
                    } else {
                        // 2FA not enabled or no email - proceed with normal login
                        session_regenerate_id(true);
